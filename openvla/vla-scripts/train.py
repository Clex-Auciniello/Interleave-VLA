...
Notes & Prerequisites:
    - If you want to set a custom location for all HF / TIMM artifacts --> `export HF_HOME="<PATH>"` *before* running!
        => For example (add to end of .bashrc): `export HF_HOME="/path/to/hf/cache"`
...
def train(cfg: TrainConfig) -> None:
    ...
    if cfg.pretrained_checkpoint is not None:
        ...
        vlm = load(cfg.vla.base_vlm, load_for_training=True)
    ...
    # DEBUG, try to re-init embedding
    num_new_tokens = action_tokenizer.n_bins
    output_embeddings = vlm.llm_backbone.llm.get_output_embeddings().weight.data 
