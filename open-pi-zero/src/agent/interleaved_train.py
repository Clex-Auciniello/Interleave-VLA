...
class InterleavedTrainAgent(TrainAgent):
    def __init__(self, cfg):
        ...
        if cfg.freeze_vision_model:
            ...
            # self.model.multi_modal_projector.requires_grad = False
        ...
        def preprocess_batch(batch, split_mask: bool, sample_fm_time: bool):
            ...
            instruction_images = [
                img[mask] for mask, img in zip(
                    ...
                    batch["task"]["interleaved_instruction"]["image_instruction"]
                )
            ]
            assert all(text.count("<image_placeholder>") == img.shape[0] for text, img in zip(texts, instruction_images))
            interleaved_images = torch.cat([torch.cat(pair, axis=0) for pair in zip(images, instruction_images)], axis=0)
            interleaved_images = einops.rearrange(interleaved_images, "B H W C -> B C H W")
                    ...
                    with torch.inference_mode():
                        for batch_eval in self.val_dataloader:
                            ...
                            if torch.isnan(preds).any() or torch.isinf(preds).any():
                                log.warning(f"NaN or Inf detected in predictions: {preds}")
