...
from src.data import interleaved_obs_transforms, interleaved_traj_transforms
...
def make_dataset_from_rlds(
    ...
    *,
    ...
    def restructure(traj):
        ...
        task = {}
        if language_key is not None:
            if language_key == "interleaved_instruction":
                if not isinstance(traj[language_key], dict):
                    raise ValueError(
                        f"Language key {language_key} must be Dict."
                    )
            else:
                raise ValueError(
                    f"Wrong dataset format for interleaved dataset."
                )
            if "interleaved_instruction" in traj:
                task["interleaved_instruction"] = traj.pop("interleaved_instruction")
            else:
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    """
    if skip_unlabeled:
        if "interleaved_instruction" not in dataset.element_spec["task"]:
            raise ValueError(
                "skip_unlabeled=True but dataset does not have language labels."
            )
        dataset = dataset.filter(
            lambda x: tf.math.reduce_any(x["task"]["interleaved_instruction"]["language_instruction"] != "")
        )
    ...
    dataset = dataset.traj_map(interleaved_traj_transforms.add_pad_mask_dict, num_parallel_calls)

    # optionally pads actions and proprio to a consistent number of dimensions
    dataset = dataset.traj_map(
        partial(
            interleaved_traj_transforms.pad_actions_and_proprio,
    ...
    dataset = dataset.traj_map(
        partial(
            interleaved_traj_transforms.chunk_act_obs,
    ...
    if train and subsample_length is not None:
        dataset = dataset.traj_map(
            partial(interleaved_traj_transforms.subsample, subsample_length=subsample_length),
            num_parallel_calls,
        )
    ...
    *,
    ...
    dataset = dataset.frame_map(
        partial(
            apply_obs_transform,
            partial(
                interleaved_obs_transforms.decode_and_resize,
    ...
    if train:
        ...
        def aug_and_dropout(frame: dict):
            ...
            dropout_fn = partial(
                interleaved_obs_transforms.image_dropout,
                seed=seed,
                dropout_prob=image_dropout_prob,
                always_keep_key=image_dropout_keep_key,
            )
            aug_fn = partial(
                interleaved_obs_transforms.augment, seed=seed, augment_kwargs=image_augment_kwargs
            )
